...
export const SplashCursor: React.FC<SplashCursorProps> = ({
  ...
  useEffect(() => {
    ...
    const pointers: PointerType[] = [createPointer()];
    ...
    function getWebGLContext(targetCanvas: HTMLCanvasElement) {
      ...
      let webglContext: any = targetCanvas.getContext('webgl2', params);
      const isWebGL2 = !!webglContext;
      if (!isWebGL2)
        webglContext =
      ...
      const halfFloatTexType = isWebGL2 ? webglContext.HALF_FLOAT : halfFloat && halfFloat.HALF_FLOAT_OES;
      let formatRGBA: any;
      let formatRG: any;
      let formatR: any;

      if (isWebGL2) {
        formatRGBA = getSupportedFormat(webglContext, webglContext.RGBA16F, webglContext.RGBA, halfFloatTexType);
        formatRG = getSupportedFormat(webglContext, webglContext.RG16F, webglContext.RG, halfFloatTexType);
        formatR = getSupportedFormat(webglContext, webglContext.R16F, webglContext.RED, halfFloatTexType);
      } else {
    ...
    function getSupportedFormat(webgl: any, internalFormat: any, format: any, type: any) {
    ...
    function supportRenderTextureFormat(webgl: any, internalFormat: any, format: any, type: any) {
    ...
    function updateColors(dt: number) {
      ...
      if (colorUpdateTimer >= 1) {
        ...
        pointers.forEach((p: PointerType) => {
    ...
    function updatePointerMoveData(pointer: PointerType, posX: number, posY: number, color: any) {
      if (!canvas) return;
      pointer.prevTexcoordX = pointer.texcoordX;
      pointer.prevTexcoordY = pointer.texcoordY;
      pointer.texcoordX = posX / canvas.width;
